...
* compare given byt array with constant offsets with supported SSL versions
* Supported SSL versions: TLS 1.0, TLS 1.1, TLS 1.2, TLS 1.3
*/
const u_char * filter_ssl_packets(const u_char*packet, const u_char *ssl_start){
    if(ssl_start[SSL_TLS_OFFSET] == TLS){
        if( 
            ssl_start[SSL_TLS_VERSION_OFFSET] == TLS1_0 || 
            ssl_start[SSL_TLS_VERSION_OFFSET] == TLS1_1 || 
            ssl_start[SSL_TLS_VERSION_OFFSET] == TLS1_2 || 
            ssl_start[SSL_TLS_VERSION_OFFSET] == TLS1_3
            ){
            return packet;
        } else {
            return NULL;
...
* takes offset to a start of 2 bytes and returns int
*/
int get_int_from_two_bytes(const u_char *ssl_start, int offset){
    return ntohs(*(uint16_t*)(ssl_start+offset));
}
...
*/
bpf_u_int32 process_packet(ssl_connection*ssl_session_map, const u_char *ssl_start, bpf_u_int32 i){
    int ssl_length = get_int_from_two_bytes(ssl_start, SSL_HEADER_LENGTH_OFFSET);
    ssl_session_map->session_bytes += ssl_length;
    i += ssl_length + FIXED_SSL_HEADER_LENGTH;
    return i;
}

/*
* Simple function to append char to string to save SNI
*/
...
char* get_SNI(const u_char* client_hello_header){
    ...
    /* extension is not SNI, try to find it and skip all other extensions*/
...
/*
* If first FIN was sent (FIN without ACK), save that information into bool value, if FIN with ACK was sent (second FIN), we close ssl connection, calculate time difference and print info, dealocate session
*/
void process_FIN_packet(tcphdr *tcph, ssl_connection *session_ptr, map<string, ssl_connection> *ssl_session_map, const struct pcap_pkthdr *header, string ID, string client_ID){
    if(session_ptr->FIN_client == true && session_ptr->FIN_client_ID != client_ID){
        if(tcph->ack == 1){
            if(session_ptr->active){
                time_diff(&session_ptr->duration, &header->ts, &session_ptr->starttime);
                print_session(session_ptr);
        ...
        session_ptr->FIN_client = true;
        session_ptr->FIN_client_ID = client_ID;
    }
}
...
void callback(u_char *ssl_sessions, const struct pcap_pkthdr *header, const u_char *packet){
    ...
    string old_client_ID = source + to_string(ntohs(tcph->source)); //client ID for SSL connection (IP+Port number)
    ...
    string client_ID = old_client_ID + old_server_ID; //save unique ID for connection client->server
    string server_ID = old_server_ID + old_client_ID; //save unique ID for connection client<-server
            ...
            if(header->caplen - i >= 5){
                ...
                if(packet[i] == SSL_CHANGE_CIPHER_SPEC || packet[i] == SSL_ALERT || packet[i] == SSL_HANDSHAKE || packet[i] == SSL_APPLICATION_DATA){
                    ...
                    if(filtered_packet != NULL){
                        const u_char *ssl_start = (const u_char*)(unsigned char*)&packet[i]; //pointer to ssl header start
                        /* check if packet is "handshake type"*/
                        if(ssl_start[SSL_CONTENT_TYPE_OFFSET] == SSL_HANDSHAKE){
                            /* if handshake type is Client Hello save all necessary info (SSL connection started)*/
                            if(ssl_start[SSL_HANDSHAKE_TYPE_OFFSET] == SSL_HANDSHAKE_CLIENT_HELLO && map_ID_pointer->client_hello == false){
                                ...
                                i = process_packet(map_ID_pointer, ssl_start, i);
                            /* process "handshake" type packet other then Client Hello */
                            } else {
                                /* if handshake type is Server Hello and connection is not already active, make it active otherwise delete connection*/
